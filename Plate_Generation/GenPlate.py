...
parser = argparse.ArgumentParser(
    description='Generates 128x32 grayscale car number plate images in \'AA 00 BB 0000\' format.')
parser.add_argument('-n', '--number', type=int, metavar='', required=True,
                    help='REQUIRED: number of images to generate')
parser.add_argument('-s', '--state', type=str, metavar='',
                    help='OPTIONAL: generate only this state code; Example: \'-s MH\' will include only Maharashtra')
...
def make_plate_images(num: int,
    ...
    if not (args.state is None):
        ...
        for i in range(len(STATE_CODES)):
            if STATE_CODES[i][0] == args.state:
                state = [args.state, STATE_CODES[i][1]]
                found_match = True

        if not found_match:
            print('Invalid state input. Usage example: -s MH')
            return
    ...
    for i in tqdm(range(num)):
        ...
        if (args.state is None):  # random valid AA
